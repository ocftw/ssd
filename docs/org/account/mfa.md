...
<small>預計操作時間：規劃與設定管理政策約 0.5–1 天，組織成員完成設定約 2–4 週</small><br><small>所需預算：無（若僅使用驗證應用程式與備份碼）；若導入安全金鑰或付費身分管理服務，需另外編列服務訂閱費</small>
...
- 服務名稱與用途（例：[Google Workspace](https://workspace.google.com/){target="_blank"}、募款平台、社群平台管理、網域與官網管理）
...
- **預設方式：驗證應用程式與備份碼**
    - 請同仁在手機安裝驗證應用程式（建議優先 [Google Authenticator](https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2){target="_blank"} 或 [Aegis Authenticator](https://getaegis.app/){target="_blank"}），登入服務後在「帳戶」或「安全性」設定裡開啟「兩步驟驗證」或「雙重驗證」，選擇使用驗證應用程式作為第二步。
    - 啟用完成後，請同仁順手產生備份碼，存在安全的地方備用（例如加密筆記、密碼管理器，或受保護的檔案）。
- **進階方式：安全金鑰或密碼金鑰**
    - 對於權限極高的帳號（例如 [Google Workspace](https://workspace.google.com/){target="_blank"} 或 [Microsoft 365](https://www.microsoft.com/microsoft-365){target="_blank"} 超級管理員、金流後台主要管理者），預算許可的話，可以改用安全金鑰或密碼金鑰，防護強度更高。
...
- [調整使用者的 2FA 設定（Google Workspace 管理說明）](https://support.google.com/a/answer/9176657?hl=zh-Hant){target="_blank"}
- [為組織設定多重驗證（Microsoft 365 管理說明）](https://learn.microsoft.com/zh-tw/microsoft-365/admin/security-and-compliance/set-up-multi-factor-authentication?view=o365-worldwide){target="_blank"}
