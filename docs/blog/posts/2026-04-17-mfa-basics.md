---
slug: mfa-basics
title: 一次性密碼、驗證器、備份碼：MFA 核心機制入門
cover_image: asserts/blog-mfa-basics.webp
date:
  created: 2026-04-17
categories:
  - 教學
...
更高等級的選項，另見本系列文章「[Passkey、FIDO2、安全金鑰：無密碼與硬體驗證全圖](2026-05-29-passkey-fido2.md)」。
...
2. **選擇驗證應用程式**：下載 Google Authenticator（[Android](https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2){target="_blank"}、[iOS](https://apps.apple.com/tw/app/google-authenticator/id388497605){target="_blank"}）或 [Aegis Authenticator](https://getaegis.app/){target="_blank"}（Android），在服務的安全設定裡選擇「驗證應用程式」或「TOTP」，掃描 QR Code 完成綁定。
3. **立刻產生並保存備份碼**：在同一頁的安全設定中找到「備份碼」選項，產生後選擇以下任一方式保存：加密的文字檔案、密碼管理器的安全備注欄位、列印後放進實體保險箱。
4. **把備份碼放到和手機分開的地方**：手機丟了，備份碼在同一台手機上是沒有用的。
5. **記錄哪些帳號用了哪個 App 的哪個條目**：特別是使用 Google Authenticator 的使用者，該 App 不支援跨裝置同步，換手機前需要手動轉移所有條目。
...
- 你的帳號屬於組織共用帳號，或你是管理員：參考本系列文章「[組織 MFA 政策設計：人員差異與風險分級](2026-05-15-org-mfa-policy.md)」。
- 你面對針對性攻擊的威脅情境（例如：記者、倡議工作者、高風險人士）：評估改用安全金鑰（FIDO2）取代 TOTP，詳見本系列文章「[Passkey、FIDO2、安全金鑰：無密碼與硬體驗證全圖](2026-05-29-passkey-fido2.md)」。
